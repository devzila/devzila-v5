<?php
$pageTitle = 'Ruby on Rails Upgrades — DevZila';
$pageDescription = 'Seamless migrations to the latest Ruby and Rails versions with minimal downtime and maximum reliability.';
$navServicesHref = '/#services';
require __DIR__ . '/partials/header.php';
?>

<header class="svc-header">
  <div class="breadcrumb"><a href="/">Home</a> · <a href="/#services">Services</a> · Ruby on Rails Upgrades</div>
  <div class="svc-label">Development</div>
  <h1>Ruby on Rails Upgrades</h1>
  <p>Keeping your Rails app up-to-date is crucial for security and performance. We handle seamless migrations to the latest Ruby and Rails versions, ensuring minimal downtime and maximum reliability.</p>
  <div class="header-actions">
    <a href="hire-us" class="btn-primary">Plan my upgrade →</a>
    <a href="#what" class="btn-ghost">What we do</a>
  </div>
</header>

<section class="svc-section" id="what">
  <h2>What we do</h2>
  <p>We de-risk upgrades by moving in safe, incremental steps — backed by tests and a clear rollback plan — so your team keeps shipping while we modernize the foundation.</p>
  <div class="feature-grid">
    <div class="feature-card">
      <div class="ic">⬆️</div>
      <h3>Version Migration</h3>
      <p>Step-by-step upgrades to the latest Ruby and Rails, following the official upgrade path safely.</p>
    </div>
    <div class="feature-card">
      <div class="ic">🔒</div>
      <h3>Security Patching</h3>
      <p>Eliminate known vulnerabilities in Ruby, Rails, and your gem dependencies.</p>
    </div>
    <div class="feature-card">
      <div class="ic">📦</div>
      <h3>Dependency Upgrades</h3>
      <p>Audit and modernize outdated gems, replacing abandoned libraries with maintained alternatives.</p>
    </div>
    <div class="feature-card">
      <div class="ic">🟢</div>
      <h3>Zero-Downtime Deploys</h3>
      <p>Roll out upgrades behind feature flags and dual-boot strategies to avoid disruption.</p>
    </div>
    <div class="feature-card">
      <div class="ic">🧪</div>
      <h3>CI Safety Net</h3>
      <p>Strengthen your test suite and CI so every step of the upgrade is verified automatically.</p>
    </div>
    <div class="feature-card">
      <div class="ic">📈</div>
      <h3>Performance Gains</h3>
      <p>Take advantage of new framework features and runtime improvements as you upgrade.</p>
    </div>
  </div>
</section>

<section class="svc-section">
  <h2>What you get</h2>
  <ul class="check-list">
    <li>A clear, prioritized upgrade roadmap with effort estimates.</li>
    <li>Incremental, reviewable pull requests — never a risky big-bang rewrite.</li>
    <li>Up-to-date dependencies free of known CVEs.</li>
    <li>A faster, more secure, and more maintainable application.</li>
  </ul>
</section>

<div class="svc-cta">
  <h2>Stuck on an old version of Rails?</h2>
  <p>Let's map out a safe path to the latest Ruby and Rails.</p>
  <div class="cta-actions">
    <a href="hire-us" class="btn-primary">Hire Us →</a>
    <a href="https://calendly.com/nilayanand/15min" target="_blank" rel="noopener noreferrer" class="btn-ghost">Book a call</a>
  </div>
</div>

<?php require __DIR__ . '/partials/footer.php'; ?>
